    ...
    def fetch_partition_information(self, table_name):
        if self.source_schema:
            sql_query = f'''
                            ...
                            WHERE
                                p.table_owner = '{self.source_schema.upper()}'
            ...
            sql_query = f'''
                            ...
                            ON
                                k.column_name = c.column_name
                            WHERE
                                p.table_owner = '{self.username.upper()}'
                                AND p.table_name = '{table_name}'
                            '''
        ...
        with self.connection.cursor() as cursor:
            ...
            for record in all_records:
                ...
                record_key = (record[0], record[1], record[3])
